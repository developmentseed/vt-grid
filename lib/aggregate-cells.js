...
function aggregateFromGrid (inputFeatures, currentTile, gridZoom, aggregations, postAggregations) {
  ...
  var cells = tiletree.getProgeny(currentTile, gridZoom)
  ...
  for (var c = 0; c < numcells; c++) {
    ...
    boxes.push(makeCell(cell, features, aggregations, postAggregations))
  }
...
function aggregateFromRaw (inputFeatures, currentTile, gridZoom, aggregations, postAggregations) {
  ...
  for (var i = 0; i < numcells; i++) {
    ...
    boxes.push(makeCell(cells[i], features, aggregations, postAggregations))
  }

  return boxes
}

function makeCell (cell, features, aggregations, postAggregations) {
  ...
  var box = {
    ...
    geometry: tilebelt.tileToGeoJSON(tiletree.toXYZ(cell))
  }
  ...
  box.properties._quadKey = tilebelt.tileToQuadkey(tiletree.toXYZ(cell))
  return box
}
